      ...
      if (!loading) {
        ...
        try {
          ...
          await campaign.methods.contribute().send({
            ...
            toast.error("Contributed got an error")
          });
          toast.success("Contributed success")
          route.replace(`/admin/campaign/${route.query.campaign}`)
        }
        catch (err) {
          console.log(err);
          handleClick();
            ...
            <CardBody>
              <Grid container >
                <Grid item xs={6} sm={6} md={6}>
                  <Button variant="contained" color="primary" onClick={() => route.push(`/admin/campaign/${route.query.campaign}/requests`)}>All requests ({summary[2]})</Button>
                </Grid>
              ...
              <Grid container >
                <Grid item xs={6} sm={6} md={6}>
                  <Button>Number of Approvers: {summary[3]}</Button >
                </Grid>
                <Grid item xs={6} sm={6} md={6} style={{ textAlign: "end" }}>
                  <Button>Campaign Balance (ether): {web3.utils.fromWei(summary[1], 'ether')}</Button >
                </Grid>
              </Grid>
